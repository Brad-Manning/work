...
int main(int argc,char** argv)
{
 ...
 //  #endif
  ...
  // UserAction classes
  //
  G4UserRunAction* run_action = new CherenkovRunAction;
  runManager->SetUserAction(run_action);
  //
  G4VUserPrimaryGeneratorAction* gen_action = new CherenkovPrimaryGeneratorAction;
  runManager->SetUserAction(gen_action);
  //
  // G4UserEventAction* event_action = new CherenkovEventAction(CherenkovRunAction* runAction);
  // runManager->SetUserAction(event_action);
  //
  G4UserStackingAction* stacking_action = new CherenkovStackingAction();
