...
int createGEANT4Files(int argc, char **argv, parameters parameter, bool useWeights, double nParticles)
{
  ...
  for (int iFile = start; iFile <= argc - 1; iFile++) {
    ...
    */
    ...
    //radius must be 2 (larger than 1.8 because scintillator hangs over edge of tank)
    double r_tank = 2;
	...
	while (theRun.findnextevent())
	  {
		...
		//1.87 = height of station (m)
		if (!(parameter.detector)) { area_tank = M_PI * pow(r_tank,2) * cos( zenith ) + 2*r_tank*1.87*sin( zenith );}
